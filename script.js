...
function calcStyle() {
  ["display", "flex-direction", "justify-content", "align-items"].forEach(
    (elName) => {
      document
        .querySelector(`#input_${elName}_fieldset`)
  ...
  ["align-self", "order", "margin-left", "margin-left", "margin-top"].forEach(
    (elName) => {
      document
        .querySelector(`#input_${elName}_fieldset`)
        .addEventListener("click", (d) => {
  ...
  ["flex-wrap", "justify-content", "align-content"].forEach((elName) => {
    document
      .querySelector(`#input_${elName}_fieldset_2`)
      .addEventListener("click", (d) => {
        jsName = changeFromCSSToJSStyle(elName);
...
const flexDirectionSelect = document.getElementById(
  "input_flex-direction_fieldset"
);
